...
| v2.0 | ログ保存先をGoogle Sheetsに変更（Streamlit Cloud再デプロイ後もデータ永続化） |
| v2.1 | 経年加算係数を概算に適用、Afterイメージ生成プロンプトへ築年数コンテキスト追加 |
...
フィードバック入力
  ...
  └─ ※ 送信時に Google Sheets へ保存（マスターモード時は非保存）
...
### Afterイメージへの築年数コンテキスト（v2.1〜）

`generate_after_image(photo_file, style_key, age='')` に `age` パラメータを追加。

- 無料モード（FLUX.1-schnell）：`AGE_PROMPT_DESC` で築年数を英語表現に変換し、FLUXプロンプトに `"renovated from a 30-40 year old building with visible aging"` の形で埋め込む。生成AIが「何年物の物件をリフォームしたか」の文脈で画像を生成するようになる。
- Proモード（FLUX.1-Kontext-dev）：実際のBefore写真から経年感が視覚的に伝わるため、プロンプト変更は不要（実施していない）。
- 将来の3D表示（Phase 4）では、築年数に応じたマテリアル・テクスチャを選択することで経年感を表現する予定。

### 経年加算係数（v2.1〜）

築年数に応じた係数を概算合計に乗算する。古い物件ほど下地処理や見えない補修が増える傾向を反映。
...
- 係数は `AGE_MULTIPLIERS` 定数で管理。実績データ蓄積後に適宜調整。
- 内訳テーブルの下に「基本概算 ＋ 経年加算内訳 → 加算後合計」を表示。
- 業者見積りとの比較（割安／適正／高め）は経年加算後の合計を基準に判定。
- 築5年未満（係数1.00）は加算なしのため表示自体が出ない。

### フィードバック学習の設計（v2.0〜 Google Sheets）
- ログ保存先：**Google Sheets**（デプロイをまたいで永続化）
- フォールバック：Secrets未設定時は `renovation_log.csv`（ローカル開発用）
- ログ列：date, area, madori, age, structure, pattern, estimated, actual, ratio, note
...
- 認証：Google Cloudサービスアカウント（JSONキー）
- Streamlit SecretsにJSONの各フィールドを `[gcp_service_account]` テーブルとして設定
- `SPREADSHEET_ID` でシートを指定
- `get_gsheet()` を `@st.cache_resource` でキャッシュ → API呼び出しを最小化

```
save_log()   → Google Sheetsに append_row()
load_log_df() → Google Sheetsから get_all_records()
分析レポート  → load_log_df() を呼ぶ → 常に最新データ
```
...
├── .gitignore                     # .env / renovation_log.csv / .streamlit/secrets.toml を除外
├── requirements.txt               # 依存ライブラリ（gspread・google-auth含む）
...
├── .streamlit/
│   └── secrets.toml               # ローカル開発用シークレット（gitignore済み）
└── renovation_log.csv             # ローカル開発フォールバック用（gitignore済み）
```
...
| `SPREADSHEET_ID` | Google SheetsのスプレッドシートID | .streamlit/secrets.toml / Streamlit Secrets |
| `gcp_service_account` | Google Cloudサービスアカウント認証情報（TOMLテーブル） | .streamlit/secrets.toml / Streamlit Secrets |
